...
class Evidence(Base):
    ...
    uploader_id = Column(BigInteger, nullable=True)
    law_firm_id = Column(BigInteger, ForeignKey('law_firms.id', ondelete='SET NULL'), nullable=True)  # 사무실 ID
    case_id = Column(BigInteger, ForeignKey('cases.id', ondelete='CASCADE'), nullable=True)  # 사건 ID
...
class Case(Base):
    ...
    id = Column(BigInteger, primary_key=True, index=True, autoincrement=False, server_default=text('get_time_id()'))  # DB DEFAULT로 시간 기반 ID 생성
    user_id = Column(BigInteger, nullable=True)  # 레거시 호환용
...
class CaseEvidenceMapping(Base):
    ...
    evidence_id = Column(BigInteger, nullable=True)
...
class EvidenceAnalysis(Base):
    ...
    id = Column(BigInteger, primary_key=True, index=True, autoincrement=False, server_default=text('get_time_id()'))  # DB DEFAULT로 시간 기반 ID 생성
    evidence_id = Column(BigInteger, nullable=True)  # 증거 ID
