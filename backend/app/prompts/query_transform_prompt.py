...
예시:
{"query": "피고인이 정보통신망에 피해자에 대한 허위사실을 게시하여 명예를 훼손하였다. 피고인은 피해자의 주거지에 무단으로 침입하였다.", "keywords": ["명예훼손", "허위사실", "주거침입", "무단침입"]}"""
...
#### 1. 죄명/청구원인 (반드시 1개 이상)
- 명예훼손, 모욕, 폭행, 상해, 협박, 주거침입, 절도, 사기, 업무방해 등

#### 2. 행위 유형 (반드시 1개 이상) ⭐ 중요
**같은 죄명이라도 구분할 수 있는 구체적 행위 방식**을 추출

**추출 원칙:**
- "어떤 방법으로" 했는지 (게시, 발언, 촬영, 유포, 침입 등)
- "어떤 특성"이 있는지 (실명, 익명, 사칭, 허위, 무단 등)
- 판례 검색 시 유사한 행위 유형의 판례를 찾을 수 있는 키워드

**예시 (참고용, 이에 국한되지 않음):**
- 명예훼손 행위 유형: 실명비방, 사칭게시, 허위사실유포, 영상게시, 캡처유포
- 모욕 행위 유형: 공개장소발언, 단체채팅방발언, 댓글작성
- 폭행 행위 유형: 대면폭행, 흉기사용, 집단폭행
- 주거침입 행위 유형: 무단침입, 야간침입, 문잠금해제
...
- 거짓말 → "허위사실"
- 몰래 촬영 → "불법촬영"
- 유튜브에 영상 올림 → "영상게시", "정보통신망"
- 피해자인 척 글 작성 → "사칭게시"

## 출력 형식 (JSON)

반드시 아래 JSON 형식으로만 출력:
{"query": "정제된 쿼리 문장", "keywords": ["죄명", "행위유형", "기타키워드"]}

**키워드 순서**: 죄명 → 행위유형 → 기타 (이 순서로 배치)"""
