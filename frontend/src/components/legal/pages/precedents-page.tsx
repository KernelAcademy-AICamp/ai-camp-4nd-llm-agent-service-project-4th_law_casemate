...
export function PrecedentsPage({ }: PrecedentsPageProps) {
  ...
  // URL 쿼리 파라미터로 자동 검색 (채팅에서 이동 시)
  useEffect(() => {
    const queryParam = searchParams.get("q");
    if (queryParam && !initialQueryProcessed.current) {
      initialQueryProcessed.current = true;
      setSearchQuery(queryParam);
      // URL에서 q 파라미터 제거 (뒤로가기 시 재검색 방지)
      setSearchParams({}, { replace: true });
      // 검색 실행
      executeSearch(undefined, 0, queryParam);
    }
  ...
  const executeSearch = async (overrideFilters?: SearchFilters, offset: number = 0, overrideQuery?: string) => {
